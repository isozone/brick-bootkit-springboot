...
[![Version](https://img.shields.io/badge/4.0.11-4.0.11-brightgreen.svg)](https://github.com/v18268185209/brick-bootkit-springboot)
...
+ springboot2项目请使用 brick-bootkit 3.x（4.x 主线仅面向 Spring Boot 3.x）
+ 修复内存泄漏问题
+ 优化部分代码结构，日志结构
+ 调整代码包结构
+ 添加指标监控 
...
> 基于 4.0.11 源码核查的功能落地状态。核心插件能力均已就绪；涉及外部中间件的高级特性代码已实装，建议在生产环境启用前完成充分验证。

### ✅ 已就绪（生产可用）
- 类加载隔离（隔离 / 共享两种模式）
- 插件生命周期热插拔（安装 / 卸载 / 启动 / 停止 / 重启）
- 插件间服务通信（RPC 与事件驱动）
- Web 控制台（监控、运维、远程调用调试）
- 监控与指标（Actuator Health、Micrometer Metrics、Prometheus）
- 安全扫描（插件包静态安全检测）
- 依赖分析与配置隔离
- 事件总线（SDK `@BrickEventListener` 自动装配）
- Maven 集成打包与依赖管理

### 🧪 高级特性（代码已实装，建议生产前充分验证）
- 集群管理（依赖 Redis 注册中心与 gRPC 通信）
- 灰度发布 / 流量路由
- 多版本插件并存
- 分布式远程调用（RemotePlugin，依赖 Redis + gRPC 中间件）
...
如果你接的是**存量老项目**（版本较老、结构不规范、担心影响现有业务），建议先看渐进式接入指南：

- [doc/8.旧项目渐进式接入指南.md](doc/8.旧项目渐进式接入指南.md)
- 配套离线体检工具：`./tools/preflight/preflight.sh <项目路径>`
  —— 不改一行代码、不加任何依赖，即可判断能不能接、从哪一级开始接

确定能接之后，下一个问题通常是"先切哪块业务"，可以交给切片分析器：

- [doc/9.业务切片分析指南.md](doc/9.业务切片分析指南.md)
- 配套分析工具：`./tools/slicer/slice.sh <项目路径>`
  —— 基于包依赖图给出候选切片的排序、对外契约类与解环提示

选定边界后，用骨架生成器把"知道切哪块"变成"得到能跑的插件"：

- [doc/10.插件骨架生成器.md](doc/10.插件骨架生成器.md)
- 配套生成工具：`./tools/scaffold/scaffold.sh <项目路径> <包名>`
  —— 自动生成 pom.xml、Bootstrap 类、契约接口、业务 stub、装配元数据

如果不想逐个运行，也可以用统一迁移向导一键串完前三步：

- [doc/11.统一迁移向导.md](doc/11.统一迁移向导.md)
- 配套向导：`./tools/migrate/migrate.sh <项目路径> --auto`
  —— 自动完成体检 → 切片分析 → 骨架生成，输出配置建议

五者按顺序用：preflight 确认能接 → 切片分析器选定边界 → 骨架生成器产出模块 → L0/L1/L2 逐级接入 → doctor 持续自检。
...
- **Spring Boot 3.x**：完全支持（推荐 3.5.5+），4.x 主线仅面向此版本线
- **Spring Boot 2.x**：不在 4.x 支持范围。继续使用请改用 brick-bootkit 3.x，
  该组合不在官方回归矩阵内，详见 [兼容与支持矩阵](doc/3.兼容与支持矩阵.md)
...
### 更新
- [4.0.11](./doc/updates/4.0.11.md) - 最新版本
- [4.0.9](./doc/updates/4.0.9.md)
